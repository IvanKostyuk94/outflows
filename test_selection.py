...
def identify_galaxy_group(gas_groups, test=True):
    ...
    group_num = select_galaxy_group(gas_groups, serra=False, test=test)
...
def get_out_gas(galaxy):
    ...
    group_gas(all_out_gas, props=None, n_peak=3, serra=False)
...
def compare_out_selection():
    ...
    # Compare the selected outflowing gas with the ground truth
    # Calculate the number of particles missing and the extra ones
    out_idces_gmm = set(out_gas_gmm["idces"])
    out_idces_true = set(galaxy_ext["out_idces"])
